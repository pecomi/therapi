...
`unlearning_pipeline.sh` chains the patient split, seed-specific baseline
training, unlearning, deletion retraining, representation evaluation, and
optional target-embedding/predictor inference stages. It reuses the existing
GDSC predictor checkpoints: only the TCGA CSG2A embeddings are regenerated for
each new aligner checkpoint.
